...
class SeqGAN(object):
    ...
    def D_train(self, real, generate, DW):
        with tf.variable_scope('loss/discriminator'):
            ...
            padding = 1e-12
            RV = -tf.reduce_mean(tf.log(real + padding))
            GV = -tf.reduce_mean(tf.log(1 - generate + padding))
            DLoss = RV + GV
            with tf.variable_scope('REGU'):
                D_REGU = sum([tf.nn.l2_loss(w) for w in DW]) * 1e-4
            total_loss = DLoss + D_REGU
            tf.summary.scalar('D_loss', total_loss)
    ...
    def build(self, REGU=1e-4):
        ...
        self.SeqGAN_train = tf.group(switch, count)
        self.clip_D = clip_D
    ...
    def do_train(self, batch):
        ...
        _, _, summary = self.sess.run([self.SeqGAN_train, self.clip_D, self.summary_op], feed_dict=feed_dict)
...
if __name__ == '__main__':
    ...
    sess = tf.Session()
    SeqGAN = SeqGAN(sess, words_num, sava_path=SAVE_PATH, D_times=5, using_cpu=USING_CPU)
    SeqGAN.build()
    SeqGAN.load()
    f = open("record.txt", 'w')
    for epoch in xrange(1, EPOCH_NUM + 1):
        for step in xrange(1, TRAIN_STEPS_NUM + 1):
            ...
            SeqGAN.do_train(sampler.next())
        the_sample = SeqGAN.gene(500)
